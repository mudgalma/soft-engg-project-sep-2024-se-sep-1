...
const fetchCompletionRate = async () => {
  try {
    const response = await fetch(
      'http://localhost:5000/admin/dashboard/statistics',
      {
        headers: {
    ...
    if (response.ok) {
      ...
      milestones_due_this_week.value =
        data.milestones_due_this_week
      milestones_completed_this_week.value =
        data.milestones_completed_this_week
      total_students.value = data.total_students

      stats.value.daily_submissions =
        data.daily_submissions

      stats.value.milestone_submission_stats =
        data.milestone_submission_stats

      stats.value.milestone_density =
        data.milestone_density

      renderDailySubmissionChart(
        stats.value.daily_submissions
      )

      renderMilestonePieChart(
        stats.value.milestone_submission_stats
      )

      renderMilestoneDensity(
        stats.value.milestone_density
      )
      ...
      console.error(
        'Failed to fetch completion rate statistics.',
        await response.text()
      )
    }
  } catch (error) {
    console.error(
      'Error fetching completion rate statistics:',
      error
    )
  }
}

const renderDailySubmissionChart = (
  daily_submissions: any[]
) => {
  ...
  new Chart(ctx, {
    type: 'line',
    data: {
      labels: daily_submissions.map(
        (s: any) => s.date
      ),
      datasets: [
        {
          label: 'Submissions',
          data: daily_submissions.map(
            (s: any) => s.count
          ),
          borderColor: '#2196f3',
          fill: false,
...
const renderMilestoneDensity = (
  milestone_density: any[]
) => {
  const ctx = document.getElementById(
    'milestoneDensityChart'
  ) as HTMLCanvasElement
  ...
  new Chart(ctx, {
    type: 'line',
    data: {
      labels: milestone_density.map(
        (s: any) => s.date
      ),
      datasets: [
        {
          label: 'Submissions',
          data: milestone_density.map(
            (s: any) => s.count
          ),
          borderColor: '#2196f3',
          fill: false,
    ...
    <div class="row">
      ...
      >
        ...
        <div class="row mb-4">
          ...
          <div class="col-md-3">
            <StatisticsCard
              title="Milestones Due This Week"
              :value="milestones_due_this_week"
              type="danger"
            />
          </div>
          ...
          <div class="col-md-3">
            <StatisticsCard
              ...
              :value="milestones_completed_this_week"
              type="success"
            />
          </div>
